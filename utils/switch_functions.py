...
from data.point_cloud_db.holo import hololens_dataset
...
def load_dataset(params):
    ...
    if  params.dataset_name in ["hololens_dataset"]:
        dataset = hololens_dataset(params,"train")
        train_size = int(len(dataset) * params.train_val_split)
        train_dataset,val_dataset = random_split(dataset, [train_size, len(dataset) - train_size])
        test_dataset = hololens_dataset(params,"test")
...
def measure_similarity(similarity_init, source_encoded, target_encoded):
    ...
    if similarity_init == "angular_distance": # Added by Hai Iluz
        a_norm = source_encoded / source_encoded.norm(dim=-1)[:, :, None]
        b_norm = target_encoded / target_encoded.norm(dim=-1)[:, :, None]
        return 1 - (torch.arccos(torch.bmm(a_norm, b_norm.transpose(1, 2)))/torch.pi) + 1e-8
