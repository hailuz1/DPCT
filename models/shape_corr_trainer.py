...
class ShapeCorrTemplate(LightningModule):
    ...
    def log_test_step_holo(self,scalar_maps):
        logs_step = {k: v for k, v in enumerate(list(scalar_maps[0]))}
        # logs_step = {v[0]: v[1] for k,v in enumerate(scalar_maps)}
        getattr(self, f"test_logs", None).append(logs_step)
        self.log_dict({f"p/{k}": v for k, v in enumerate(list(scalar_maps[0]))}, on_step=False, on_epoch=True)
        #self.log_dict({f"{v[0]}": v[1] for k, v in enumerate(scalar_maps)}, on_step=False, on_epoch=True)

    def log_test_step_holo_list(self,scalar_maps):
        logs_step = {k: int(v) for k, v in enumerate(scalar_maps)}
        getattr(self, f"test_logs", None).append(logs_step)
        self.log_dict({f"{k}": int(v) for k, v in enumerate(scalar_maps)}, on_step=False, on_epoch=True)
    ...
    def compute_acc(label, ratio_list, soft_labels, p,input2,track_dict={},hparams=Namespace()):
        ...
        pred_hit_2 = Tensor(assignment)
        pred_hit_2 = pred_hit_2.type(torch.IntTensor)
        
        #p_new = -p
        #min_v = p_new.min()
        #ppp = p_new + (-min_v) + 1e-08
        #arr_1 = ppp.detach().cpu().numpy()
        #arr = dlib.matrix(arr_1[0])
        #assignment = dlib.max_cost_assignment(arr)
        #pred_hit_2 = Tensor(assignment)
        #pred_hit_2 = pred_hit_2.type(torch.IntTensor)
        ...
        track_dict["acc_mean_dist"] = target_dist[pred_hit_2.long(),hit].mean().item()
    ...
    def _prob_to_corr_test(prob_matrix):
        ...
        arr_1 = prob_matrix.detach().cpu().numpy()
        arr = dlib.matrix(arr_1[0])
        assignment = dlib.max_cost_assignment(arr)
        idx_2 = Tensor(assignment)
        ...
        idx_2 = idx_2.type(torch.IntTensor).long()
        idx_2 = idx_2.view([1,1024,1])

        for bsize in range(c.shape[0]):
            for each_row in range(c.shape[1]):
                c[bsize][each_row][idx_2[bsize][each_row]] = 1.0
