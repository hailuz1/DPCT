...
from models.sub_models.PCT.model_new import Pct, Point_Transformer_Last
...
class DeepPointCorr(ShapeCorrTemplate):
    ...
    def __init__(self, hparams, **kwargs):
        ...
        self.model_type_for_encoder = hparams.model_type_for_encoder
        if self.model_type_for_encoder == 'DGCNN':
            self.encoder = DGCNN_MODULAR(self.hparams, use_inv_features=self.hparams.use_inv_features)
        elif self.model_type_for_encoder == 'PCT':
            #self.encoder = Pct(self.hparams)
            self.encoder = DGCNN_MODULAR(self.hparams, use_inv_features=self.hparams.use_inv_features)
    ...
    def configure_optimizers(self):
        ...
        #self.optimizer = torch.optim.NAdam(self.parameters(), lr = self.hparams.lr)
        ...
        #self.optimizer = torch.optim.AdamW(self.parameters(), lr=0.00003, weight_decay=self.hparams.weight_decay)

        #self.optimizer = torch.optim.SGD(self.parameters(), lr=0.01*100, momentum=0.9, weight_decay=3e-4)
        #self.optimizer = torch.optim.SGD(self.parameters(), lr=0.01*100, momentum=0.9)
        #self.optimizer = optimizer = torch.optim.RMSprop(self.parameters(), lr=0.0001, weight_decay=5e-4) 
        #self.scheduler = MultiStepLR(self.optimizer, milestones=[10, 30], gamma=0.5) #hai


        self.scheduler = MultiStepLR(self.optimizer, milestones=[3], gamma=0.2) #hai


        
        #self.scheduler = CosineAnnealingLR(self.optimizer, 5, eta_min=0.01) #hai


        #self.scheduler = MultiStepLR(self.optimizer, milestones=[6, 9], gamma=0.1) #hai
        return [self.optimizer]#, [self.scheduler] #hai
    ...
    def compute_deep_features(self, shape):
        if self.model_type_for_encoder == 'DGCNN':
            ...
            #shape["dense_output_features"] = self.encoder(shape["pos"])
            shape["dense_output_features_t"] = self.encoder.forward_per_point(
                shape["pos"], start_neighs=shape["neigh_idxs"])
            shape["dense_output_features"] =  (self.pt_last(shape["dense_output_features_t"].permute(0,2,1),shape["pos"])).permute(0,2,1)
        return shape
    ...
    def forward_source_target(self, source, target):
        ...
        # for source["dense_output_features"].size() = [8, 512, 1024] I get P_non_normalized.size() = [8, 512, 512]
        # for source["dense_output_features"].size() = [8, 1024, 512] I get P_non_normalized.size() = [8, 1024, 1024]
    ...
    def get_neighbor_loss(source, source_neigh_idxs, target_cross_recon, k):
        ...
        target_cr_grouped = grouping_operation(target_cross_recon.transpose(1, 2).contiguous(), neigh_index_for_loss.int()).permute(0, 2, 3, 1) # PCT: target_cr_grouped: [8, 1024, 10, 3]
    ...
    def test_step(self, test_batch, batch_idx):
        ...
        argmax_idx = 1
        list_of_index = p.argmax(dim=argmax_idx).detach().cpu().numpy() if len(p.shape) == 3 else p
        arr_1 = p.detach().cpu().numpy()
        arr = dlib.matrix(arr_1[0])
        assignment = dlib.max_cost_assignment(arr)
        #array_of_ass = np.array(assignment.items())  # TODO: I want to return the cost of each pair and do threshold to ignore false pairs

        # new_p = p.detach().cpu().numpy().squeeze()
        # list_of_index = []
        # inx_th = np.argwhere(new_p < 0)
        # new_p[inx_th[0],inx_th[1]] = 0
        # for i in range(new_p.shape[0]):
        #     col_ind = np.argmax(np.max(new_p,0))
        #     row_ind = np.argmax(np.max(new_p,1))
        #     new_p[:,col_ind] = -np.inf
        #     new_p[row_ind,:] = -np.inf
        #     list_of_index.append([row_ind,col_ind])
        #     if np.argwhere(new_p > -np.inf).size == 0:
        #         break
       
        _ = self.compute_acc(label, ratio_list, soft_labels, p, input2, track_dict=self.tracks,hparams=self.hparams)
        #self.log_test_step_holo(list_of_index)

        #self.log_test_step_holo_list(assignment)
